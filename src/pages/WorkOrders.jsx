...
function exportToExcel(data) {
  const headers = ['#','Title','Category','Priority','Store','Assigned To','Status','Created Date','SLA']
  const rows = data.map((wo, i) => [
    i + 1,
    wo.title || '',
    wo.title?.split('—')[0]?.trim() || '',
    wo.priority || '',
    wo.stores?.name || '',
    wo.tech_name || 'Unassigned',
    wo.status?.replace('_',' ') || '',
    wo.created_at ? new Date(wo.created_at).toLocaleDateString() : '',
    wo.status === 'closed' ? 'Done' : 'Active',
  ])
  ...
  link.href = url
  link.download = `Fixnar_WorkOrders_${new Date().toISOString().split('T')[0]}.csv`
  link.click()
